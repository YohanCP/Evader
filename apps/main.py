...
# Load sprite player 1, player 2, peluru, dan shield
...
# Fungsi untuk merotasi gambar dengan mempertahankan alpha channel
...
# Ukuran player (gunakan ukuran gambar yang sudah dirotasi)
PLAYER_TARGET_W = 100
...
# Ukuran peluru yang baru
AMMO_W = 80
AMMO_H = int(AMMO_W * ammo_img_raw.shape[0] / ammo_img_raw.shape[1])

# Healthbar awal (gunakan global agar bisa di-reset saat restart)
health_player1 = 100
health_player2 = 100
...
# Waktu terakhir shield dinonaktifkan untuk cooldown tambahan
...
def deactivate_shield(player_id_str):
    global shield_active_player1, shield_active_player2, \
           last_shield_deactivation_time_p1, last_shield_deactivation_time_p2
    time.sleep(3) # Durasi shield aktif
...
def main():
    ...
    cv2.namedWindow("Two Player Blink Shot")
    cv2.setMouseCallback("Two Player Blink Shot", handle_mouse_event)
    ...
    while True:
        ...
        if current_game_state == GAME_STATE_PLAYING:
            ...
            if results_face.multi_face_landmarks:
                ...
                for face_landmarks in results_face.multi_face_landmarks[:2]:
                    ...
                    # Menentukan player_id berdasarkan posisi hidung relatif terhadap garis tengah
                    ...
                    # Menyesuaikan posisi X untuk penempatan gambar yang lebih baik
                    if player_id == 0: # Player 1 (kiri)
                    ...
                    # Pastikan hanya satu pemain terdeteksi per sisi layar jika dua wajah terdeteksi
                ...
                for player in detected_players:
                    ...
                    if blinking and eye_ready_to_blink[player_id] == 1 and current_time - last_blink_time[player_id] >= BLINK_COOLDOWN:
                        ...
                        # Posisi awal proyektil agar keluar dari moncong kapal
                        if player_id == 0:
                            ammo_start_x = player["x"] + PLAYER_TARGET_W - (rotated_ammo_w // 2)
                        else: # Player 2 menembak ke kiri
            ...
            for i in range(len(projectiles) - 1, -1, -1):
                ...
                for idx, position in enumerate(player_positions):
                    if position is not None:
                        ...
                        # Koordinat kotak pembatas proyektil
                        proj_left = proj['x']
                        proj_top = proj['y']
                        ...
                        if (proj_left < player_right and proj_right > player_left and
                            ...
                            # Pastikan proyektil tidak mengenai pemain yang menembaknya sendiri
                            if proj['player'] != f"Player {idx + 1}":
                                # Periksa status perisai
                                if (idx == 0 and shield_active_player1) or \
                                   (idx == 1 and shield_active_player2):
                                    collision_occurred = True
                                    break 
                                else:
                                    # Tabrakan berhasil dan tidak ada perisai aktif
                                    if idx == 0:
                                        health_player1 -= 10
                                    else:
                                        health_player2 -= 10
                ...
                # Hapus proyektil jika terjadi tabrakan
                if collision_occurred:
                    projectiles.pop(i) 

            # Gambar shield jika aktif (hanya di mode PLAYING)
            for idx, position in enumerate(player_positions):
                if position is not None:
                    player_x, player_y = position["x"], position["y"]
                    ...
                    # Sesuaikan posisi agar perisai tetap di tengah pesawat
        ...
        # --- Draw Instructions with Fade Out ---
        ...
        if current_game_state == GAME_STATE_WINNER:
            ...
            # Posisi tombol REPLAY dan CLOSE (lebih naik)
